...
%DIF DEL main-PNAS_OLD.tex   Tue Apr  8 17:24:53 2025
%DIF ADD main-PNAS.tex       Tue Apr  8 18:12:06 2025
...
\DIFdelend \DIFaddbegin \author[a,d]{\DIFaddend Andrew Baggaley}
\author[b,c]{Wei Guo}
\DIFdelbegin %DIFDELCMD < \author[d,a]{%%%
\DIFdelend \DIFaddbegin \author[e,a]{\DIFaddend Luca Galantucci}
\DIFdelbegin %DIFDELCMD < \author[e]{%%%
\DIFdelend \DIFaddbegin \author[f]{\DIFaddend Giorgio Krstulovic}
...
\DIFdelbegin %DIFDELCMD < \affil[d]{Istituto per le Applicazioni del Calcolo ``M. Picone" IAC CNR, Via dei Taurini 19, 00185 Roma, Italy}
%DIFDELCMD < \affil[e]{Universit\'e C\^ote d'Azur, Observatoire de la C\^ote d'Azur, CNRS,Laboratoire Lagrangre, Boulevard de l'Observatoire CS 34229 - F 06304 NICE Cedex 4, France}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affil[d]{Department of Mathematics and Statistics, Lancaster University, Lancaster, LA1 4YF, United Kingdom}
...
\DIFaddbegin \DIFadd{We note that the reconnection event also generates wavepackets of Kelvin waves about either side of the reconnection cusp, which propagate outward (visible in the bottom panel of Fig~\ref{fig:ring-coll-viz}). These waves are the fundamental mechanism transfering superfluid kinetic energy to smaller scales \mbox{%DIFAUXCMD
\cite{vinen2001decay,vinen2002quantum}}\hskip0pt%DIFAUXCMD
. The interaction of Kelvin waves with the normal fluid has recently been studied \mbox{%DIFAUXCMD
\cite{stasiakCrossComponentEnergyTransfer2024,stasiak2025inverse}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{exp-snap-1.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[b]{0.24\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{exp-snap-2.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD <     \begin{subfigure}[b]{0.24\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{exp-snap-3.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD <     \begin{subfigure}[b]{0.24\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{exp-snap-4.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\hfill
%DIFDELCMD <        \vspace{5pt}
%DIFDELCMD < 	\begin{subfigure}[b]{0.24\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{snap-1.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[b]{0.24\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{snap-2.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD <     \begin{subfigure}[b]{0.24\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{snap-3.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD <     \begin{subfigure}[b]{0.24\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics*[width=\textwidth]{snap-4.pdf}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < %%%
...
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{exp-snap-1.pdf}
%DIF >  	\end{subfigure}
%DIF >  	\begin{subfigure}[b]{0.24\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{exp-snap-2.pdf}
%DIF >  	\end{subfigure}
%DIF >      \begin{subfigure}[b]{0.24\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{exp-snap-3.pdf}
%DIF >  	\end{subfigure}
%DIF >      \begin{subfigure}[b]{0.24\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{exp-snap-4.pdf}
%DIF >  	\end{subfigure}
%DIF >  	\hfill
%DIF >         \vspace{5pt}
%DIF >  	\begin{subfigure}[b]{0.24\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{snap-1.pdf}
%DIF >  	\end{subfigure}
%DIF >  	\begin{subfigure}[b]{0.24\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{snap-2.pdf}
%DIF >  	\end{subfigure}
%DIF >      \begin{subfigure}[b]{0.24\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{snap-3.pdf}
%DIF >  	\end{subfigure}
%DIF >      \begin{subfigure}[b]{0.24\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{snap-4.pdf}
%DIF >  	\end{subfigure}
%DIF >  \caption{\emph{Top row:} Images showing tracer particles trapped on reconnecting vortices in superfluid helium at 1.65 K. The arrows denote the vortices before and after the reconnection. The first two images show that the vortices approach each other before the reconnection, which occurs at $t=0.58$ s. After the reconnection, the resulting vortices start to move apart, as shown in the last two images.
%DIF >  \emph{Bottom row:} Oblique collision of two circular vortex rings at different 
%DIF >  (dimensionless) times, here in units of $\tau=0.183$s.
%DIF >  The superfluid vortex lines are represented by red tubes (the radius has been greatly 
%DIF >  exaggerated for visual purposes); the scaled normal fluid enstrophy 
%DIF >  $\bom^2/\bom^2_{max}$ is represented by the blue volume rendering.}
%DIF >  \label{fig:ring-coll-viz}
%DIF >  \end{figure*}
\DIFaddbegin 
...
%DIF >  \begin{figure}[t]
%DIF >  	\centering
%DIF >  	\begin{subfigure}[b]{0.45\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{min-delta-with-exp.pdf}
%DIF >  		\caption{}
%DIF >  		\label{fig:minimum-distance}
%DIF >  	\end{subfigure}
%DIF >  	\begin{subfigure}[b]{0.45\textwidth}
%DIF >  		\centering
%DIF >  		\includegraphics*[width=\textwidth]{prefactors-with-exp.pdf}
%DIF >  		\caption{}
%DIF >  		\label{fig:prefactors}
%DIF >  	\end{subfigure}
%DIF >  \caption{\emph{(a)}: Time evolution of the (dimensionless)
%DIF >  minimum distance squared $\delta^2$ plotted versus (dimensionless) $\kappa (t-t_0)$ for
%DIF >  the Hopf link reconnections at $T=0K,1.9K$ and $2.1K$ (black, blue and red
%DIF >  respectively). The grey shaded areas
%DIF >  are the regions used to estimate the prefactors $A^{\pm}$. 
%DIF >  \emph{Inset:} Experimental superfluid helium data.
%DIF >  \emph{(b)}: Comparison of all prefactors: Hopf links (\emph{HL}, circles), 
%DIF >  ring collisions (\emph{RC}, stars with yellow outline), GPE-data 
%DIF >  from Villois \emph{et. al.} \cite{villoisIrreversibleDynamicsVortex2020} (green diamonds) and experimental results from this study (triangles and vertical dot-dashed lines). 
%DIF >  The shaded areas associated with each colour represent the convex hull of errors 
%DIF >  for each temperature and the black dashed line represents the theoretical bound for $A^+/A^-$. Schematic rendering of initial conditions are included.}
%DIF >  \end{figure}
\DIFaddbegin 

\DIFaddend \subsection*{Scaling law}
...
%DIF > \red{Above the vortex line density threshold, the perturbation of normal fluid energy generated by the reconnection is still present when the successive reconnection takes place: the energy does not decay to zero, but will increase in time. 
%DIF > Below this critical threshold, in some situations, even a background level of normal  fluid disturbances which remains small but is constantly injected by reconnections may become relevant. 
%DIF > In general, these finite amplitude normal fluid disturbances constantly injected by reconnections may become relevant for various superfluid helium systems. 
%DIF > One example are oscillatory flows, widely studied  in superfluid helium using vibrating wires and forks. At low frequency, perturbations which start at finite-amplitude rather than infinitesimal-amplitude level  have enough time to became of order one (hence visible and destabilizing) in the supercritical part of the cycle \cite{barenghi1989modulated}. A second example is pipe flow, again relevant to helium experiments, which is known to suffer from finite-amplitude instabilities \cite{peixinho2007finite}. This effect clearly needs further investigations.} 

\DIFaddbegin \DIFadd{Above the vortex line density threshold, the increase of normal fluid energy generated by the reconnection will not have time to decay before the subsequent reconnection occurs, which will add again more energy. In this manner, the normal fluid energy will not decay to zero, but will increase in time. In general, such finite amplitude normal fluid disturbances constantly injected by reconnections may become relevant for various superfluid helium systems. One example is the oscillatory flows, which are widely studied in superfluid helium using vibrating wires and forks. At low frequency, perturbations which start at finite-amplitude rather than infinitesimal-amplitude level have enough time to become of order one (hence visible and destabilizing) in the supercritical part of the cycle \mbox{%DIFAUXCMD
\cite{barenghi1989modulated}}\hskip0pt%DIFAUXCMD
. A second example is pipe flow, again relevant to helium experiments, which is known to suffer from finite-amplitude instabilities \mbox{%DIFAUXCMD
\cite{peixinho2007finite}}\hskip0pt%DIFAUXCMD
. This effect clearly needs further investigations.
}
...
$T=0~\rm K$ case. Blue and red circle correspond to $T=1.9K$ and $T=2.1K$ \DIFdelbeginFL \DIFdelFL{,}\DIFdelendFL respectively. The purple squares are from GPE simulations of
...
%DIFDELCMD < \subsection*{Experimental Method}
%DIFDELCMD < 

%DIFDELCMD < To visualize the reconnection 
%DIFDELCMD < dynamics, we decorate the vortices using solidified deuterium 
%DIFDELCMD < ($\rm{D_2}$) tracer particles of density $202.8~\rm{kg/m^3}$ 
%DIFDELCMD < \cite{xu2008density}) and mean radius $1.1\times10^{-6}~\rm m$ 
%DIFDELCMD < \cite{tang2023visualization,tang2021visualization}. These particles
%DIFDELCMD < are generated by injecting a $\rm D_2$/${}^{4}\rm He$ gas mixture into 
%DIFDELCMD < the superfluid helium bath 
%DIFDELCMD < %through a gas injection system 
%DIFDELCMD < \cite{tang2023visualization,fonda2016injection} 
%DIFDELCMD < as described in the Supplementary 
%DIFDELCMD < Material (SM).
%DIFDELCMD < %\cite{SeeSupplementaryMaterials} for more details). 
%DIFDELCMD < When the particles are near the vortices,
%DIFDELCMD < they become trapped inside their cores
%DIFDELCMD < because of the Bernoulli pressure arising from the 
%DIFDELCMD < circulating superfluid flow.
%DIFDELCMD < % around the vortex cores. 
%DIFDELCMD < A thin laser sheet is used to illuminate the particles, and their motion 
%DIFDELCMD < is recorded at 200 Hz by a camera positioned at a right angle to the laser 
%DIFDELCMD < sheet. A high-quality reconnection event, observed at $T=1.65~\rm K$ 
%DIFDELCMD < and capturing both the pre- and post-reconnection dynamics, is shown 
%DIFDELCMD < in  Fig.~\ref{fig:ring-coll-viz} as an example. Note that according 
%DIFDELCMD < to GP simulations \cite{GiuriatoQuantumVortexReconnections2020} the transfer 
%DIFDELCMD < of energy and momentum between particle and vortex does not modify
%DIFDELCMD < the approaching rates significantly. Reconnection events reported in this
%DIFDELCMD < work have been captured in multiple experiments, either following particle 
%DIFDELCMD < injection or long time (\textit{i.e.}, 30-60 s) after towing a grid through superfluid helium. 
%DIFDELCMD < % Whereas in the particle-vortex system, vortex energy and momentum 
%DIFDELCMD < %are also transferred to particles, these additional mechanisms do not 
%DIFDELCMD < %modify the approaching rates 
%DIFDELCMD < (this is also supported by the scaling symmetry 
%DIFDELCMD < of the system which allows to draw conclusion for length-scales relevant 
%DIFDELCMD < to experiments). 
%DIFDELCMD < 

%DIFDELCMD < \subsection*{Numerical Method}
%DIFDELCMD < 

%DIFDELCMD < We follow the approach of Schwarz \cite{schwarz1988} which exploits
%DIFDELCMD < the vast separation of length scales between the vortex core $a_0$ and any
%DIFDELCMD < other relevant distance, in particular the average distance between vortices,
%DIFDELCMD < $\ell$, in the case of turbulence. Vortex lines are described
%DIFDELCMD < as space curves $\s(\xi,t)$ where $\xi$ is arclength. The equation of motion 
%DIFDELCMD < of the vortex lines is
%DIFDELCMD < \begin{displaymath}
%DIFDELCMD < \label{eq:s}
%DIFDELCMD < 	\dot{\s}(\xi,t) = \v_s + \frac{\beta}{(1+\beta)}\left[\v_{ns}\cdot \s'\right]\s' + \beta\s'\times\v_{ns}+\beta'\s'\times\left[\s'\times \v_{ns}\right],
%DIFDELCMD < \end{displaymath}
%DIFDELCMD < %
%DIFDELCMD < where $\dot{\s}=\partial\s/\partial t$, $\s'=\partial\s/\partial \xi$ 
%DIFDELCMD < is the unit tangent vector, 
%DIFDELCMD < $\v_n$ and $\v_s$ are the normal fluid and superfluid velocities at $\s$,
%DIFDELCMD < $\v_{ns}=\v_n - \v_s$, and $\beta$, $\beta'$ are temperature and Reynolds number dependent 
%DIFDELCMD < mutual friction coefficients \cite{galantucciNewSelfconsistentApproach2020b}. The normal fluid velocity $\v_n$ is described as a classical fluid obeying the incompressible ($\nabla\cdot\v_n=0$) Navier-Stokes equations:
%DIFDELCMD < %
%DIFDELCMD < \begin{displaymath}
%DIFDELCMD < 	\frac{\partial \v_n}{\partial t} + (\v_n\cdot\nabla)\v_n = 
%DIFDELCMD <         -\frac{1}{\rho} \nabla p + \nu_n\nabla^2\v_n + \frac{\mathbf{F}_{ns}}{\rho_n},
%DIFDELCMD <         \label{eq:vn}
%DIFDELCMD < \end{displaymath}
%DIFDELCMD < where $\mathbf{F}_{ns}$ is the mutual friction force that couples the normal fluid and the superfluid vortices, and acts as an internal injection mechanism. In Eq.~\eqref{eq:vn}, $\rho=\rho_n + \rho_s$, where $\rho_n$ and $\rho_s$ are the normal fluid and superfluid densities, $p$ is the pressure, 
%DIFDELCMD < and $\nu_n$ is the kinematic viscosity of the normal fluid. 
%DIFDELCMD < Equations~(\ref{eq:s}) and (\ref{eq:vn})
%DIFDELCMD < are solved in dimensionless form by rescaling them by the characteristic time $\tau$ and length $\lambda$. 
%DIFDELCMD < The algorithm for vortex reconnections is standard
%DIFDELCMD < \cite{baggaleySensitivityVortexFilament2012a}. We consider two distinct initial vortex configurations
%DIFDELCMD < at three temperatures $T=0~\rm K$, $1.9~\rm K$ and $2.1~\rm K$
%DIFDELCMD < corresponding to the superfluid fractions $\rho_s/\rho=100 \%$,
%DIFDELCMD < $58 \%$ and $26 \%$. To compare with experiments, the unit of length is set to $\lambda=1.59 \times 10^{-4}~\rm m$, and the time units to $\tau=0.183~\rm s$ at $T=0~\rm K$ and $1.9~\rm K$, and $\tau=0.242~\rm s$ at $T=2.1~\rm K$, 
%DIFDELCMD < see also the SM for details.
%DIFDELCMD < %\cite{SeeSupplementaryMaterials} for details.
%DIFDELCMD < All configurations lead to a vortex reconnection.
%DIFDELCMD < The first configuration consists of two vortex rings 
%DIFDELCMD < of (dimensionless) radius $R\approx 1$ in a tent-like shape
%DIFDELCMD < which collide obliquely 
%DIFDELCMD < making an initial angle $\alpha$ with the vertical direction, 
%DIFDELCMD < as shown in Fig.~\ref{fig:ring-coll-viz}, and,
%DIFDELCMD < schematically, in Fig.~\ref{fig:prefactors}.   
%DIFDELCMD < By changing the parameter $\alpha$, we create a sample of 12 realizations
%DIFDELCMD < at each temperature (again, see the SM for details).
%DIFDELCMD < %\cite{SeeSupplementaryMaterials} for details).
%DIFDELCMD < %We take 12 realizations of $\alpha$, such that 
%DIFDELCMD < %$\alpha\in\lbrace i\pi/13|i=1,\cdots,12\rbrace$. 
%DIFDELCMD < %The supplementary material 
%DIFDELCMD < %gives more details of our model and these configurations
%DIFDELCMD < %\cite{SeeSupplementaryMaterials}. 
%DIFDELCMD < The second configuration is the Hopf link, shown schematically in Fig.~\ref{fig:prefactors}. It consists of two perpendicular linked rings of radius $R\approx1$ with an offset in the $xy$-plane.
%DIFDELCMD < By changing the offset, we create a sample of 49 reconnections
%DIFDELCMD < at each temperature, as described in the SM.
%DIFDELCMD < %\cite{SeeSupplementaryMaterials}.
%DIFDELCMD < %defined by parameters $\Delta l_x$ and $\Delta l_y$ 
%DIFDELCMD < %The offsets are chosen so that 
%DIFDELCMD < %$(\Delta l_x, \Delta l _y) \in \lbrace(0.125i,0.125j)|i,j=-3,\cdots,3 \rbrace$,
%DIFDELCMD < %providing us with a total of 49 reconnections for each temperature. 
%DIFDELCMD < In all cases, normal fluid structures generated by moving superfluid vortex rings \cite{kivotides-barenghi-samuels-2000}, are initially prepared to eliminate any potential transients.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \bibsplit[19]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibsplit[21]
\DIFaddend %Use \bibsplit to split the references from the body of the text. Value "[2]" represents the number of reference in the left column (Note: Please avoid single column figures & tables on this page.)
